...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		# If the current index is less than the number of children, at least one untested child remains.
		if self.getCurrentIndex() < self.getNumChildren():
			#print "sequence", self.id, self.getCurrentIndex(), self.getNumChildren()
			# Check for the current child's success.
			success = self.getChild(self.getCurrentIndex()).execute()
			# If success, increment current index and return None. If failure, return False. If running, return None.
			...
			return None
		# If current index equals or exceeds the number of children, all children have succeeded; return True.
...
class Selector(BTNode):
	...
	def execute(self, delta = 0):
		...
		# If the current index is less than the number of children, at least one non-failure child remains.
		if self.getCurrentIndex() < self.getNumChildren():
			#print "selector", self.id, self.getCurrentIndex(), self.getNumChildren()
			# Check for the current child's success.
			success = self.getChild(self.getCurrentIndex()).execute()
			# If success, return True. If failure, increment current index and return None. If running, return None.
			if success is not None:
				if success:
					return True
				else:
					self.setCurrentIndex(self.getCurrentIndex() + 1)
			return None
		# If current index equals or exceeds the number of children, all children have failed; return False.
